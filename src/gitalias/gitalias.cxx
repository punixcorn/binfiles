...
auto stringToLower(std::string &str) -> std::string {
  auto view = str | std::ranges::views::transform(
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    desc.add_options()("help,h", "print this message")(
        ...
        "add an origin")("repo,r", opt::value<string>(&G->reponame),
                         "name for creating an online repo * [i]")(
        "Des,D", opt::value<string>(&G->repodes),
        "description for the online repo * [ii]")(
        "type,t", opt::value<string>(), "if repo should be private * [iii]")(
    ...
    if (args.count("push")) {
      ...
      if (arg("push", string).size() > 0) {
        if (arg("push", string) == "all") {
          ...
          Isubcommand(G, " && git push -u origin ", arg("push", string));
        }
      }
    }
    ...
    if (args.count("Visibility")) {
      if (arg("Visibility", vector<string>).size() != 2) {
        exitWithoutHelp("1Invalid arguments\nUsage: --Visibility <repoName> "
                        "<private/public>");
      }
      ...
      if ((viewType != "private") && (viewType != "public")) {
        exitWithoutHelp("2Invalid arguments\nUsage: --Visibility <repoName> "
                        "<private/public>");
      }

      if (!findFile("/bin", "curl"))
        exitWithoutHelp("Could not find dependency : curl..."
                        "\nTerminating...\n");

      UserInfo User = ParseUserInfo();
      if (User.username == "_" || User.token == "_") {
        exitWithoutHelp("Username and Token not set in config "
      ...
      Vstr = fmt::format(
          ...
          "/dev/null",
          User.username, User.token, viewType == "private" ? "true" : "false",
      ...
      while (fgets(buffer, 999, instance) != null) {
        if (strstr(buffer, "Bad credentials") != null) {
          exitWithoutHelp("Bad credentials : Please update your token, it may "
                          "have "
                          "expired");
        } else if (strstr(buffer, "Could not resolve host: api.github.com") !=
                   null) {
          exitWithoutHelp("Could not connect to api.github.com\n"
    ...
    if (args.count("Clone")) {
      ...
      if (arg_count == 3) {
        ...
        if (atoi(arg("Clone", vector<string>).back().c_str()) == 0)
          exitWithoutHelp("Clone takes in only 3 args max, The folder to "
                          "clone "
    ...
    if (args.count("origin")) {
      ...
      T->origin = true;
      std::string otemp{
          fmt::format(" && git remote add origin git@github.com:{}.git && git "
                      "branch -M "
    ...
    if (args.count("type")) {
      auto repotype = arg("type", string);
      string repo = stringToLower(repotype);
      if (repo == "private") {
        G->mode = true;
      } else if (repo == "public") {
        G->mode = false;
        ...
        T->repoMode = false;
        exitWithoutHelp("type can only be public or private");
      }
      T->repoMode = true;
    }
    ...
    if (args.count("Grab")) {
      ...
      if (!lcheck)
        exitWithoutHelp("Could not find dependency : Svn... Terminating");
...
auto Isubcommand(Globals *g, const string_view &s1, const string_view &s2)
  ...
  if (s2.size() != 0)
    g->subcommand += std::string(s2.data());
}
...
[[nodiscard("grab UserInfo from file")]] auto ParseUserInfo() -> UserInfo {
  ...
  if (!FileToWorkWith.is_open()) {
    fileCreated = true;
    fmt::print("Cannot open config file, file does not exist. Creating new "
               "file..\n");
  ...
  if (FileToWorkWith.is_open() && !FileToWorkWith.bad() &&
    ...
    try {
      ...
      try {
        userinfo.defaultMessage = pt.get<std::string>("default_message");
      } catch (...) {
  ...
  */
  ...
  if (userinfo.token.size() == 0 || userinfo.username.size() == 0) {
    exitWithoutHelp("Invalid values obtain from config file "
...
auto getGitInfo() -> bool {
  ...
  if (fd == null || temp == null)
    exitWithoutHelp("Program Crashed...");
...
auto createOnlineRepo(Globals *g) -> void {
  ...
  if (token == "_") {
    exitWithoutHelp("Token value not set\nEdit "
                    "~/.config/gitalias/githubuserinfo.json\n");
  }
  ...
  /* consent lol */
  fmt::print("Online repository details:\n"
             "\tName: {}\n"
             "\tDescription : {}\n"
             "\tVisibility: {}\n"
             "\n\n"
  ...
  while (fgets(buffer, 999, instance) != null) {
    if (strstr(buffer, "Bad credentials") != null) {
      exitWithoutHelp("Bad credentials : Please update your token, it may have "
                      "expired");
    } else if (strstr(buffer, "Could not resolve host: api.github.com") !=
               null) {
      exitWithoutHelp("Could not connect to api.github.com\n"
