  ...
  try {
    ...
    if (args.count("Visibility")) {
      if (arg("Visibility", vector<string>).size() != 2) {
        exitWithoutHelp("Invalid arguments\nUsage: --Visibility <repoName> "
                        "<private/public>");
      }
      ...
      if (arg("Visibility", vector<string>)[(index_of_reponame == 1 ? 0 : 1)] !=
              "private" &&
          arg("Visibility", vector<string>)[(index_of_reponame == 1 ? 0 : 1)] !=
              "public") {
        exitWithoutHelp("Invalid arguments\nUsage: --Visibility <repoName> "
                        "<private/public>");
      };
    ...
    if (args.count("Clone")) {
      ...
      if (tsize == 2) {
        ...
        exitWithoutHelp("--Clone / -C takes in only 2 args max, The folder to "
                        "clone "
...
[[nodiscard("grab UserInfo from file")]] auto ParseUserInfo() -> UserInfo {
  ...
  if (!FileToWorkWith.is_open()) {
    fileCreated = true;
    fmt::print("Cannot open file, file does not exist. Creating new file..\n");

    // checking if folder exists
    if (!std::filesystem::exists(std::string(
            std::string(getenv("HOME")).append("/.config/gitalias")))) {
      std::filesystem::create_directory(
          std::string(std::string(getenv("HOME")).append("/.config/gitalias")));
    };
...
auto checkStaged() -> bool {
  ...
  if (fd == null || temp == null)
    exitWithoutHelp("Program Crahsed...");
...
auto createOnlineRepo(Globals *g) -> void {
  ...
  if (token == "_") {
    exitWithoutHelp(
        "Token value not set\nEdit ~/.config/gitalias/githubuserinfo.json\n");
  }
