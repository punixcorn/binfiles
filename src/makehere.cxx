 ...
 * [ sample ] : makehere -p project-name -l [cpp/c] -C [make/cmake] -std [11/14/17/20/2a/2x] -f
 * <filename>
 */
...
struct Engine
{
  ...
  private:
    ...
    void buildClangd()
    {
        ...
        std::string str = fmt::format("CompileFlags:\n"
                                      "\tAdd: [ -std={}{} ]\n",
    ...
    void buildMake()
    {
        ...
        {
            std::filesystem::copy("~/.Makefile", fmt::format("./{}/Makefile", project));
            return;
        }

        std::string makestr{fmt::format("CC={}\n"
                                        "FILE={}\n"
                                        "STD=-std={}{}\n"
                                        ...
                                        "\t@rm main.o main\n"
                                        "\t@echo cleaning done",
                                        (language == "c" ? "gcc" : "g++"), filename, (language == "c" ? "c" : "c++"),
                                        standard)};
    ...
    void buildCmake()
    {
        ...
        std::string cmakestr{
            fmt::format("cmake_minimum_required(VERSION 3.5)\n"
                        "project({0})\n"
                        "add_executable(${{PROJECT_NAME}} {1})\n"
                        "set_target_properties(${{PROJECT_NAME}} PROPERTIES {2}_STANDARD {3})\n"
                        "set(CMAKE_{2}_STANDARD_REQUIRED true)\n"
                        "#find_package(fmt)\n"
                        "#target_link_libraries(${{PROJECT_NAME}} PRIVATE fmt::fmt-header-only -static)\n"
                        "set(EXECUTABLE_OUTPUT_PATH ${{CMAKE_SOURCE_DIR}}/bin)\n",
                        project, filename, (language == "c" ? "C" : "CXX"), standard)};

        std::string make_for_cmake = fmt::format("CC=cmake\n"
                                                 "FILE={}\n"
                                                 "main: build\n"
                                                 "\t$(CC) --build build/\n\n"
                                                 "build: ${{FILE}}\n"
        ...
        if (Templates)
        {
            if (std::filesystem::exists("~/.CMakeLists.txt-c") && language == "C")
            {
                std::filesystem::copy("~/.CMakeLists.txt-c", fmt::format("./{}/CMakeLists.txt", project));
            }
            else if (std::filesystem::exists("~/.CMakeLists.txt-cpp") && language == "CXX")
            {
                std::filesystem::copy("~/.CMakeLists.txt-c", fmt::format("./{}/CMakeLists.txt", project));
            }
            else
  ...
  public:
    ...
    void parse()
    {
        ...
        default:
            ...
            while (i < Argc)
            {
                if (isOptionFound(Args[i], "--language"))
                {
                    ...
                    lowerCase(language);
                    if ((language != "c") && (language != "cpp") && (language != "cxx") && (language != "c++"))
                    {
                ...
                {
                    ++i;
                    check_next(Args[i], "-f / --filename needs an argument");
                    filename = string(Args[i]);
                }
                else if (isOptionFound(Args[i], "--compiler"))
                {
                    ++i;
                    check_next(Args[i], "-c / --compiler needs an argument");
        ...
        else
        {
            ...
            {
                if (filename.find(".cpp") == std::string::npos && filename.find(".cxx") == std::string::npos &&
        ...
        if (language == "c") //  check for template file for c
        {
            ...
            {
                std::filesystem::copy("~/.template.c", fmt::format("./{}/{}", project, filename));
            }
            else
            {

                createFile(filename, fmt::format("/*\n"
                                                 "project: {} \n"
                                                 "author: \n"
                                                 "*/\n"
                                                 "#include<stdio.h>\n"
        ...
        else //  check for template file for cpp
        {
            ...
            {
                std::filesystem::copy("~/.template.cpp", fmt::format("./{}/{}", project, filename));
            }
            else
            {
                createFile(filename, fmt::format("/*\n"
