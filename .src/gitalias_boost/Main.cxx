...
void
Isubcommand(const string_view& s1, const string_view& s2);
/* prints error message e with help message and kills the program [ ret: void
 * ]*/
auto
errorT1(const string_view& e) -> void;
/* prints error message e and kills the program  [ ret: void ]*/
auto
errorT2(const string_view& e) -> void;
/* check is something exists at place [ ret: bool ]*/
auto
getdir(const string& place, const string_view& thing) -> bool;
...
main(int argc, char* argv[])
{
    ...
    try {
        ...
        desc.add_options()("help,h", "print this message")(
          ...
          "create a branch")(
          "switch,s", opt::value<string>(), "switch to a branch")(
          ...
          "Request,R",
          opt::value<string>()->default_value("https"),
          "Protocol to use when cloning, [ https/ssh ]")(
          "verbose,v", "print out parsed code")("log,l", "show log files")(
          "Status,S",
          "show statuts")("origin,o", opt::value<string>(), "add an origin")(
          "repo,r",
          opt::value<string>(&reponame),
          "name for creating an online repo * [i]")(
          "Des,D",
          opt::value<string>(&repodes),
        ...
        if (args.count("message")) {
            ...
            for (vector<string>::const_iterator i =
                 ...
                 i != args["message"].as<vector<string>>().end();
                 i++) {
                messagebox += " -m '";
                messagebox += *i;
                messagebox += "' ";
                ...
                if (tempstr.find("https://github.com") != string::npos ||
                    ...
                    if (args["Request"].as<string>() == "ssh") {
                        ...
                        cout << program_invocation_name
                             << ": using default Protocol for cloning : "
                                "https...\n";
...
auto
Isubcommand(const string_view& s1, const string_view& s2) -> void
{
    subcommand += s1;
    if (s2.length() != 0) subcommand += s2;
}
...
parse(Trips* t) -> void
{
    ...
    if (t->commit) {
        ...
        if (!t->add) {
            /* if no added files */
            if (!Checkadd()) addbox += " && git add . "; // add all
        }
        ...
        if (t->add) parseCommand += addbox;
    }
    if (parseCommand.length() > 0) command += parseCommand;
